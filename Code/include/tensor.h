...
public:
  ...
  Tensor operator+(
      const float scalar) const; // add a scalar to each element of the tensor
  ...
  float &operator()(size_t row,
                    size_t col); // access element by index (non-const)
  const float &operator()(size_t row,
                          size_t col) const; // for const access  function
